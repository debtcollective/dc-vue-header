...
export const getProfileLink = () => `${discourseEndpoint()}/u/${currentUsername()}`;

export const getInboxLink = () => `${getProfileLink()}/messages`;

export const getCurrentUser = () =>
  fetch(`${discourseEndpoint()}/about/live_post_counts.json`, getOptions()).then(res => {
    if (res.status === 403) {
...
export const logout = () =>
  fetch(`${discourseEndpoint()}/session/${currentUsername()}`, deleteOptions()).catch(() =>
